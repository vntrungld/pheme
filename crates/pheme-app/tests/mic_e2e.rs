...
/// reconnecting client's microphone. Demand travels on a `watch` channel: `session()`
/// calls `mic.wanted()` — a fresh clone of the same long-lived receiver — at the top of
/// every call, including a reconnect, and a fresh clone starts already caught up to the
/// sender's *current* version, so its `changed()` only resolves on a version it has not
/// seen yet. After a reconnect, where the client's virtual microphone never stopped
/// reporting `Wanted`, the demand poller has had no reason to change the value since
/// before the reconnect, so the fresh clone is caught up from the moment it is born and
/// `changed()` never fires. Only the unconditional post-handshake send reopens the far
/// end's microphone. Without it, a user whose network blipped mid-recording finds the
/// microphone shut for good, with no counter and no log line saying why.
///
