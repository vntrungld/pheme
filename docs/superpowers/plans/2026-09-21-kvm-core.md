...
- Commit messages follow `{Update|Fix|WIP|Hotfix}: {short description}` + body + `Co-Authored-By: Claude Opus 5 (1M context) <noreply@anthropic.com>` (see `~/.claude/CLAUDE.md`).
...
- QUIC: ALPN `pheme/1` (main) and `pheme-pair/1` (pairing); default port 24800; `max_idle_timeout = 5 s`, `keep_alive_interval = 1 s`, `initial_rtt = 1 ms`, `datagram_send_buffer_size = 16 KiB`, `datagram_receive_buffer_size = 64 KiB`, `max_concurrent_bidi_streams = 4`.
