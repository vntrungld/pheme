...
**Tech Stack:** Rust 2021, `pipewire` 0.8 (Linux), `windows` 0.62 / WASAPI (Windows), `rubato` 0.16 (drift resampling), `rtrb` 0.3 (lock-free rings), `crossbeam-channel`, `quinn` datagrams via the existing `pheme-net`.
...
**API caveat:** the listing below targets `pipewire` 0.8. If the installed version spells
something differently, adapt the call and keep the behaviour: a node named "Pheme Speaker"
